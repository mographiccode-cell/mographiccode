...
def hash_prob(u):
    ...
    z=M['char_intercept']; w=M['char_coef']
...
def lex_prob(u):
    ...
    for i,v in enumerate(x):
        scale=M['lex_scale'][i] or 1.0
        z+=M['lex_coef'][i]*((v-M['lex_mean'][i])/scale)
    return sigmoid(z)

def ml_prob(u):
    cw,lw=M.get('blend',[0,1]);raw=cw*hash_prob(u)+lw*lex_prob(u)
    # Re-center so the validation-selected threshold becomes 0.5 risk.
    th=float(M.get('threshold',.5))
...
def score_url(u,sender_domain=''):
    risk=ml_prob(u);ml=risk;reasons=[];rawp=raw_split(u);host=(rawp.hostname or '').lower();netloc=(rawp.netloc or '').lower();nu=normalize_url(u)
    try:ipaddress.ip_address(host);risk=max(risk,.95);reasons.append('ip-host')
    except:pass
    if '@' in netloc:risk=max(risk,.995);reasons.append('userinfo-at')
    if host.startswith('xn--') or '.xn--' in host:risk=max(risk,.90);reasons.append('punycode')
    try:
        if rawp.port and rawp.port not in (80,443):risk=max(risk,.84);reasons.append('nonstandard-port')
    except ValueError:
        risk=max(risk,.90);reasons.append('invalid-port')
    ...
    nh=nested_hosts(u)
    if any(regdom(x)!=regdom(host) for x in nh):risk=max(risk,.84);reasons.append('cross-domain-redirect')
    sender_reg=regdom(sender_domain)
    for brand in OFFICIAL_URL:
        if brand in host and not is_official_host(host,brand):
            official_nested=any(is_official_host(x,brand) for x in nh)
            sender_is_official=sender_domain==OFFICIAL_SENDER.get(brand) or sender_domain.endswith('.'+OFFICIAL_SENDER.get(brand,''))
            if sender_is_official and official_nested:
                risk=max(risk,.44);reasons.append('external-tracker-to-official')
            else:
                risk=max(risk,.985);reasons.append('brand-domain-mismatch:'+brand)
    official_any=any(is_official_host(host,b) for b in OFFICIAL_URL)
    hard=any(r in reasons for r in ('ip-host','userinfo-at','punycode','nonstandard-port','invalid-port'))
    if official_any and not hard:risk=min(risk,.16);reasons.append('official-domain')
    if sender_reg and regdom(host)==sender_reg and not hard and not any(r.startswith('brand-domain-mismatch') for r in reasons):
...
def score_meta(c):
    text=c['text'];sender_dom=email_domain(c.get('sender',''));auth=parse_header(text,'Authentication-Results').lower()
    ...
    risk=.30;reasons=[];trusted_brand=None
    ...
    for b,d in OFFICIAL_SENDER.items():
        if b in frm and not (sender_dom==d or sender_dom.endswith('.'+d)):
            risk=max(risk,.94);reasons.append('sender-brand-mismatch:'+b)
    return risk,trusted_brand,reasons

def credential_request(text):
    low=(text or '').lower()
    if re.search(r'(enter|provide|submit|type).{0,55}(password|recovery code|verification code|one-time code|otp|mfa code)',low,re.S):return True
    return False
...
def advanced_predict(c):
    text_score,ok=call_text(c);sender_dom=email_domain(c.get('sender',''));urls=extract_urls(c['text'])
    urows=[score_url(u,sender_dom) for u in urls];max_url=max([x[0] for x in urows] or [.10]);mean_url=sum(x[0] for x in urows)/len(urows) if urows else .10
    meta,brand,mreasons=score_meta(c);all_reasons=[r for x in urows for r in x[2]];hard=[r for r in all_reasons if r in ('ip-host','userinfo-at','punycode','invalid-port') or r.startswith('brand-domain-mismatch')]
    final=.60*max_url+.12*mean_url+.20*meta+.08*text_score
    if hard:final=max(final,.92)
    if max_url>=.90:final=max(final,.88)
...
def main():
    ...
    result={'benchmark':'PhishLens advanced fusion v3','base':metrics(rows),'holdout':metrics(hrows),'base_mistakes':[r for r in rows if r['pred']!=r['truth']],'holdout_mistakes':[r for r in hrows if r['pred']!=r['truth']],'url_model_test_metrics':M.get('test_metrics'),'url_model_threshold':M.get('threshold'),'url_model_blend':M.get('blend')}
